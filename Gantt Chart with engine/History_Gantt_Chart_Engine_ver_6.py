...
def start(dataset_id: str, simulation_prefix: str):
    ...
    try1 = Analysis("V_F00001", "V_F_00004")
...
class Analysis:
    ...
    # @staticmethod
    # def call_customdata(bar):
    #     temp = list(bar.customdata)
    #     temp = temp[0][1]
    #     return temp
    ...
    def generate_fig_target(self):
        ...
        self.fig_target = px.timeline(self.df, x_start='Start', x_end="Finish", y="Task", text="target", color="target",
                                      hover_name="event", labels={'Task':'lot_id'}, width=2000, height=750, template='seaborn',
                                      hover_data=['lpst'])
        # 파일 저장에는 width 2300이 적당하지만 show에서는 2000이 적당하다
        # height=900, height=750

        for lot_id in self.id_list:
            self.fig_target.add_annotation(x=Analysis.str_to_datetime(self.id_finish[lot_id] + 13000), y=lot_id,
                                    text=f"{self.facility_utiliztion_rate[lot_id]:.1f}%",
                                    showarrow=False)


        self.fig_target.update_layout(bargap=0.1)
        self.fig_target.update_traces(textfont_size=12, textangle=270, textposition='inside', offset=False)
    ...
    def generate_fig_event(self):
        ...
        self.fig_event = px.timeline(self.df, x_start='Start', x_end="Finish", y="Task", text="target", color="event",
                                   hover_name="target", color_discrete_map=col_dict, labels={'Task':'lot_id'}
                                   , width=2000, height=750, hover_data=["lpst"])

        for lot_id in self.id_list:
            self.fig_event.add_annotation(x=Analysis.str_to_datetime(self.id_finish[lot_id] + 13000), y=lot_id,
                                    text=f"{self.facility_utiliztion_rate[lot_id]:.1f}%",
                                    showarrow=False)

        self.fig_event.update_layout(bargap=0.1, plot_bgcolor='#D6DBDF')
    ...
    def analysis_engine_result(self, *args):
        ...
        if 'target' in args:
            ...
            self.fig_target.write_html(f"{PathInfo.xlsx}{os.sep}temp_target.html")
            self.fig_target.show()

        if 'event' in args:
            self.generate_fig_event()
            self.fig_event.write_html(f"{PathInfo.xlsx}{os.sep}temp_event.html")
            self.fig_event.show()