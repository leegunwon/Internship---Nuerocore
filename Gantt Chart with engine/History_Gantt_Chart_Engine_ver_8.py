...
class Analysis:
    ...
    def modify_hov_name_from_cus_data(bar):
        ...
        """
        bar.hovertext = bar.customdata[0]
    ...
    def update_layout_target(self):
        ...
        with self.fig_target_col.batch_update():
            ...
            [Analysis.modify_opacity(bar, 0.6) for bar in self.fig_target_col.data if
             (' ' in bar.legendgroup)]
            [Analysis.modify_hover_name(bar) for bar in self.fig_target_col.data if
             (' ' in bar.legendgroup)]
    ...
    def analysis_engine_result(self, *args):
        ...
        if 'target' in args:
            ...
            self.fig_target_col.show(width=2000, height=750, config={'modeBarButtonsToAdd':['Zoom']})
