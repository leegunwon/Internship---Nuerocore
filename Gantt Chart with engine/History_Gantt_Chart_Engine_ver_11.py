...
def start(dataset_id: str, simulation_prefix: str):
    ...
    try1 = Analysis()
    try1.analysis_engine_result_lot_history('treemap', 'sankey')
    t2 = time.time()
    print(t1-t0, t2-t1)
...
class Analysis:
    ...
    def __init__(self, **kwargs: list):
        ...
        self.fig_sankey_diagram = None
        self.df_sankey_data = None
        self.df_late = None
        self.label_name = None

        if 'prod_id' in kwargs.keys():
            if kwargs['prod_id'] != None:
        ...
        with self.fig_target_col.batch_update():
            ...
            self.fig_target_col.add_trace(
                go.Bar(base=self.df_late.start, x=self.df_late.operation_time, y=self.df_late.lot_id, orientation='h',
                       name='late', legendgroup='late', opacity=0.6, marker=dict(color='black'), hovertext=self.df_late.lpst))
    ...
    def generate_sankey_dataframe(self, treemap=bool):
        """
        현재 prod명과 다음 prod명을 source와 target에 분류하고, prod개수를 1로 설정하여 데이터프레임을 작성.
        """
        if self.prodID == []:
            data = [dict(source=prod.currProd, target=prod.nextProd, value=1, dmdID=prod.dmdProdID)
        ...
        if treemap == True:
            self.df_treemap = self.df_sankey_data
    ...
    def generate_treemap_dataframe(self, sankey=bool):
        if sankey == False:
            if self.prodID == []:
                data = [dict(source=prod.currProd, target=prod.nextProd, value=1, dmdID=prod.dmdProdID)
                        for prod in ComDataInventory.orpDataInv.orpDepthInfoList if prod.nextProd != prod.currProd]
        ...
        new_df = self.df_treemap[self.df_treemap['target'] == self.df_treemap['dmdID']]
        new_df = new_df.copy()
        new_df.loc[:, 'source'], new_df.loc[:, 'target'] = new_df['target'], ''
        self.df_treemap = self.df_treemap.sort_values(by='source', ascending=True)
        self.df_treemap = self.df_treemap.append(new_df, ignore_index=True)


    def generate_fig_treemap(self):
        self.fig_treemap = go.Figure(go.Treemap(labels=self.df_treemap['source'], parents=self.df_treemap['target'], values=self.df_treemap['value']))
    ...
    def analysis_engine_result_lot_history(self, *args):
        ...
        if 'sankey' in args:
            if 'treemap' in args:
                self.generate_sankey_dataframe(treemap=True)
                ...
                self.generate_sankey_dataframe(treemap=False)
                self.generate_fig_sankey_diagram()
                self.fig_sankey_diagram.write_html(f"{PathInfo.xlsx}{os.sep}temp_sankey.html")
                self.fig_sankey_diagram.write_image(f"{PathInfo.xlsx}{os.sep}temp_sankey.png")
                self.fig_sankey_diagram.show()
